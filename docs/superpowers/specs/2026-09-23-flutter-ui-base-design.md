...
| 91 | On the empty Library, "Browse starter decks" is disabled without the spec A4 "not available yet" hint: `MxEmptyState` has no slot for a hint on its secondary action. Every other waiting control on screen 01 carries it — closed: `MxEmptyState.secondaryActionHint` carries it | Impeccable review (critique + audit) |
