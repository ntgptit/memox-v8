...
| D17 | **`MxStatTile`.** A new shared widget: a large tabular number over a small-caps label, the figure the screen acts on in primary and any other (a zero included) in plain ink, as the kit draws New and Due, one semantics node "{label}: {value}". Callers: the entry hero (New, Due) and the summary hero (three stats). Built under `flutter-theme-design`, with its widget test and goldens. | Owner, 2026-09-26: two callers exist (14, 21). |
