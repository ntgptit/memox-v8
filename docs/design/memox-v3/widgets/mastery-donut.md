...
## Accessibility contract — MasteryDonut

Accessibility — requirement, value. The screen reader reads this contract, not the pixels:
  role                FIXED           exposed as one element whose accessible value is the percentage the label shows (for example "42 percent"); the arc and track are excluded from semantics
  colour              FIXED           the value is in the accessible value, so the band colour is never the only carrier of the meaning
...
modes, three shapes:
  ...
  track                             progress-track  ·  M3_ALIAS
      ...
      the unfilled ring
  arc · < 34%                       status-learning  ·  MEMOX_SEMANTIC_COLOR
      access: DIRECT · treatment: FULL_STRENGTH
      the low band arc
  label · < 34%                     status-learning-text  ·  MEMOX_SEMANTIC_COLOR
      access: DIRECT · treatment: FULL_STRENGTH
      the low band label — the same amber family as the arc, in its text-grade ink (#8F5800 light / #FFC658 dark). The fill amber is 2.15:1 as text on a light surface, so number and ring share the hue family, not the identical value
...
progress-track · status-learning · status-learning-text · status-reviewing · status-mastered
