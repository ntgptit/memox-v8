...
## Accessibility contract — IconButton

Accessibility — requirement, value. The screen reader reads this contract, not the pixels:
  accessible name     REQUIRED        every instance takes a name that states its action; an unnamed instance is a build error. The painted glyph is excluded from the semantics tree so the control is read once
  tooltip             REQUIRED        the long-press label equals the accessible name
  role                FIXED           button; the disabled state is exposed as disabled
