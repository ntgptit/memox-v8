...
Dimensions — label, dimension class, value:
  ...
  touch area          MINIMUM         48 tall — a centred band around each painted option; the tray (40) and thumb (32) are not grown. Adjacent option bands meet at the middle of the 2 gap and never overlap; one grouped control, so the 8 spacing between separate targets does not apply between its options
...
## Self-check — SegmentedTray
  ...
  ownership               PASS
  web mechanics           PASS
  touch geometry          PASS — the touch area row gives every option a 48 band around the painted 32
  long content            PASS
  loading width           n/a
  UNSPECIFIED             none
