...
## Accessibility contract — AppBar

Accessibility — requirement, value. The screen reader reads this contract, not the pixels:
  leading accessible name   REQUIRED  the leading icon button takes a name that states its action (back or close)
  action accessible name    REQUIRED  every trailing action takes a name; a menu trigger says it opens a menu
  title               FIXED           exposed as a heading; the title text is the screen name for the screen reader
