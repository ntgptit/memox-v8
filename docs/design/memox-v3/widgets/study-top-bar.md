...
Dimensions — label, dimension class, value:
  ...
  progress track      FIXED           height 4, radius full, progress-track
...
here:
  ...
  onSurfaceVariant · progress-track

## Accessibility contract — StudyTopBar

Accessibility — requirement, value. The screen reader reads this contract, not the pixels:
  close accessible name  REQUIRED     the close control takes a name that states its action; the glyph is excluded from semantics
  progress value      FIXED           exposed as "N of M" (the accessible value), never as the fill length; the 4 track itself is excluded from semantics
  counter             FIXED           the counter text is the same value, so it is not announced twice

Caller-owned copy is passed in as the accessible name; this component never derives it from a glyph name.

## Reduced motion — StudyTopBar

The 200ms progress fill growth is removed; the fill jumps to its new width and the value is still exposed as N of M. Global rule: "Reduced motion" in the Foundations spec.
