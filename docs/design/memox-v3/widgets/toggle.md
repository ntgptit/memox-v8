...
Dimensions — label, dimension class, value:
  ...
  track off           FIXED           outline (3:1 or better against page and card)
...
here:
  ...
  op-disabled · shadow-soft · surface-bright · outline · onSurface

## Accessibility contract — Toggle

Accessibility — requirement, value. The screen reader reads this contract, not the pixels:
  role                FIXED           exposed as a switch; the state is exposed as On or Off and changes are announced
  accessible name     CALLER-OWNED    the label of the row it sits in; the track and thumb are excluded from semantics
  disabled            FIXED           exposed as disabled, with no on/off change possible

Caller-owned copy is passed in as the accessible name; this component never derives it from a glyph name.

## Reduced motion — Toggle

The 160ms thumb travel and track colour change are removed; the on or off state applies at once. Global rule: "Reduced motion" in the Foundations spec.
...
modes, three shapes:
  ...
  slot / state                      binding
  track · off                       outline  ·  M3_COLOR
      access: DIRECT · treatment: FULL_STRENGTH
      the resting track — outline, so the off state clears 3:1 against the page (surfaceContainerHighest was 1.25:1 in light)
  track · on                        primary  ·  M3_COLOR
      access: DIRECT · treatment: FULL_STRENGTH
      the engaged track
  thumb                             surfaceBright (light) / onSurface (dark)  ·  M3_COLOR
      access: DIRECT · treatment: FULL_STRENGTH
      the sliding thumb, lifted by shadow-soft. On the dark outline track the thumb is onSurface (4.15:1); surfaceBright would be 2.65:1
...
outline · primary · surfaceBright · onSurface · shadow-soft · op-disabled
