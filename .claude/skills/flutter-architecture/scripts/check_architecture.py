...
# Generated files with a plain name carry the generator's banner near the top,
# as drift_dev's `schema_versions.dart` does ("GENERATED BY drift_dev, DO NOT
# MODIFY."). The banner, not the name, is what says a hand never edits it.
_GENERATED_BANNER = re.compile(r"GENERATED.*DO NOT MODIFY", re.IGNORECASE)
_BANNER_LINES = 10
...
def _dart_files(lib: Path, repo_root: Path) -> list[tuple[str, list[str]]]:
    ...
    for path in sorted(lib.rglob("*.dart"), key=lambda p: p.as_posix()):
        ...
        if any(fragment in rel for fragment in _GENERATED_DIRS):
            continue
        if any(_GENERATED_BANNER.search(line) for line in lines[:_BANNER_LINES]):
            continue
