...
void main() {
  test('deck -> card -> two reviews -> reset leaves a consistent database', () async {
    ...
    // Every invariant query of schema.md still returns no row, but 25: as
    // schema.md writes it, it has no generation filter, so the scheduled turn
    // a reset keeps (above) trips it once the card starts over. It holds again
    // once schema.md scopes it with `a.generation = s.generation`.
    const unscopedByGeneration = {25};
    for (final MapEntry(key: number, value: query)
        in invariantQueries.entries) {
      if (unscopedByGeneration.contains(number)) continue;
