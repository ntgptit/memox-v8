import 'package:memox/features/study/di/study_session_repository_provider.dart';
import 'package:memox/features/study/domain/usecases/show_fill_hint_use_case.dart';
import 'package:riverpod_annotation/riverpod_annotation.dart';

part 'show_fill_hint_use_case_provider.g.dart';

@riverpod
ShowFillHintUseCase showFillHintUseCase(Ref ref) =>
    ShowFillHintUseCase(ref.watch(studySessionRepositoryProvider));
